...
def score_clause_risk(contract_name: str, clause_info: dict, provider: str, model_name: str, api_key: str, session_id: str = None) -> dict:
    ...
    system_prompt = (
        ...
        "- MEDIUM: Ambiguous language, partial mismatch, or an unaddressed security/compliance-relevant term.\n"
        "- LOW: Fully compliant, or a non-security commercial term that policy silence doesn't make risky.\n\n"
        "GROUNDING RULES (critical):\n"
        "- If the CONTRACT CLAUSE specifies a concrete number, timeframe, or frequency, scan the ENTIRE "
        "retrieved context for the sentence addressing that SAME parameter -- a [Policy Result] block "
        "often covers several sub-topics, so don't stop at the first topically-related sentence.\n"
        "- Base your justification ONLY on the RETRIEVED COMPANY POLICY CONTEXT. Do not cite external "
        "laws or 'typical'/'industry standard' figures unless that exact figure appears verbatim in the context.\n"
        "- If policy is silent on the clause's specific parameter: score MEDIUM only if that parameter is "
        "security/compliance-relevant (e.g. encryption, breach response, retention, audit rights, liability). "
        "Score LOW if it's general commercial boilerplate (e.g. governing law, standard termination notice) "
        "unrelated to data protection or security. Never infer or estimate a missing figure.\n"
        "- Quote or closely paraphrase the specific retrieved sentence and name which [Policy Result N] it came from.\n\n"
